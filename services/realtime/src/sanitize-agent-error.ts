...
export type SanitizedAgentError = {
  /** Safe message for the channel bubble */
  publicMessage: string
  /** Short machine code for logs / traces */
  code:
    | 'rate_limit'
    | 'auth'
    | 'model'
    ...
    | 'network'
    | 'generic'
  /** Operator detail (already redacted) for server logs */
  detail: string
}
...
export function sanitizeAgentError(agentName: string, reason: unknown): SanitizedAgentError {
  const msg =
  ...
  if (
    ...
    return {
      publicMessage: `${agentName} could not load the fleet runner. Check AGENTS_REPO_ROOT and agents install on the host.`,
      code: 'module',
      detail,
    }
  }
